...
const destinations = await sequelize.query(
  ...
  "st.store_name, st.store_code, st.organization_level, st.address, " +
  ...
  "JSON_AGG(JSON_BUILD_OBJECT(" +
    ...
    "'status', b.status" +
  ") ORDER BY COALESCE(b.split_level, 0) ASC, b.id ASC) AS batch_nodes " +
  "FROM public.inventory_batches b " +
  "LEFT JOIN public.stores st ON st.id = b.current_organization_id " +
  "WHERE (b.root_batch_id = :root_batch_id OR b.id = :root_batch_id) " +
  "AND COALESCE(b.available_qty, 0) > 0 " +
  "GROUP BY b.current_organization_id, st.store_name, st.store_code, st.organization_level, st.address " +
